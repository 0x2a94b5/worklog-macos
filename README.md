...
导入时，缺少的状态默认为“未完成”，分类按标题推断，备注为空，创建和更新时间使用导入时的当前时间。导入只追加到目标月份；如需修改已有任务，请在清单中双击编辑。
...
App 每天首次启动时生成一份完整 SQLite 备份并保留最近 14 份；数据库升级前额外备份，菜单“工作项 > 备份数据库”可手动备份。Markdown 导出是便于阅读和交换的文本副本，不代替完整数据库备份。
